...
namespace SIGA_PET.Controllers
{
    ...
    /// **ATENÇÃO: Esta operação irá DELETAR TODOS OS DADOS e recriar o banco do zero!**
        ...
        /// - 👨‍⚕️ 3 Funcionários (Veterinário, Tosador, Atendente)
        ...
        public async Task<IActionResult> ResetarEPopular()
        {
            try
            {
                ...
                {
                    new Venda { UsuarioId = usuarios[4].UsuarioId, TutorId = tutores[0].TutorId, FuncionarioId = funcionarios[2].FuncionarioId, DataVenda = DateTime.Now.AddDays(-10), ValorTotal = 209.80m, FormaPagamento = "Cartao de Credito", Observacoes = "Compra de racao e petisco" },
