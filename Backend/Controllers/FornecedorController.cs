...
namespace SIGA_PET.Controllers
{
    ...
    public class FornecedorController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<FornecedorDto>>> SearchFornecedores([FromQuery] string name)
        {
            ...
            var fornecedores = await _context.Fornecedores
                .AsNoTracking()
                // Corre��o: Verifica se RazaoSocial n�o � nulo antes de chamar Contains
                .Where(f => f.Nome.Contains(name) || (f.RazaoSocial != null && f.RazaoSocial.Contains(name)))
                .ToListAsync();

            if (!fornecedores.Any())
            {
                return NotFound("Nenhum fornecedor encontrado com o nome ou raz�o social fornecida.");
            }
        ...
        // DELETE: api/Fornecedor/5 - COM VERIFICAÇÃO DE PRODUTOS
        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteFornecedor(int id)
        {
            try
            {
                ...
                {
                    return BadRequest($"Não é possível excluir o fornecedor pois existem {fornecedor.Produtos.Count} produto(s) associado(s). Remova os produtos ou altere seus fornecedores primeiro.");
                }
